...
class Trainer:
    ...
    def _train_step(
        ...
        loss, batch_log, prompt_trigger_dic_train, prompt_trigger_dic_val = model(batch, clean_prompt, trigger, target, prompt_trigger_dic_train, prompt_trigger_dic_val)
        loss.backward()
        self.train_op()
        ...
        for epoch in range(total_train_epochs):
            for step, batch in enumerate(train_dataloader):
                batch_log, prompt_trigger_dic_train, _ =\
                    self._train_step(step, batch, config['clean_prompt'], config['trigger'], config['target'], prompt_trigger_dic_train, prompt_trigger_dic_val)
                if report_to_wandb:
                    wandb.log(batch_log)
                total_steps += 1
